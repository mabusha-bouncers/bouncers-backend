...
from src.utils.utils import get_payment_methods, get_plan_scheduled_terms, get_scheduled_term_days
import re
import phonenumbers
...
class PropertySetters(Util):
    ...
    @staticmethod
    def set_string(prop: ndb.StringProperty, value: Optional[str]) -> Optional[str]:
        """
    ...
    @staticmethod
    def set_float(prop: ndb.FloatProperty, value: Optional[float]) -> float:
        """
    ...
    def set_currency(prop: ndb.StringProperty, value: Optional[str]) -> str:
        ...
        """
        from src.currency_symbols.currencies import currency_util
        property_name: str = property_.return_property_name(prop=prop)
        if value is None:
            raise TypeError(f"{property_name} , cannot be Null")
